...
fn validate_face_data(face: &FaceData, tol: f64) -> bool {
    ...
    for loop_data in &face.loops {
        ...
        let loop_poly = trim_loop_polyline(loop_data);
        if loop_poly.len() < 4 {
            return false;
        }
        for idx in 1..loop_data.edges.len() {
            if uv_distance(
                loop_data.edges[idx - 1].end_uv,
                loop_data.edges[idx].start_uv,
            ) > tol
            {
                return false;
            }
...
fn build_trim_loop_from_points(
    points: &[RgmUv2],
    is_outer: bool,
    tol: f64,
) -> Result<TrimLoopData, RgmStatus> {
    if points.len() < 3 {
...
fn build_trim_loop_from_edges(
    state: &SessionState,
    loop_input: RgmTrimLoopInput,
    edges: &[RgmTrimEdgeInput],
    tol: f64,
) -> Result<TrimLoopData, RgmStatus> {
    if edges.is_empty() {
        return Err(RgmStatus::InvalidInput);
    }
    if loop_input.edge_count as usize != edges.len() {
        return Err(RgmStatus::InvalidInput);
    }

    let mut built_edges = Vec::with_capacity(edges.len());
    for edge in edges {
        if uv_distance(edge.start_uv, edge.end_uv) <= tol && !edge.has_curve_3d {
            continue;
        }
        let curve = if edge.has_curve_3d {
            Some(edge.curve_3d)
        } else {
            None
        };
        let uv_samples = if let Some(curve_handle) = curve {
            sample_trim_edge_curve_uv(state, curve_handle, edge.start_uv, edge.end_uv, tol)?
        } else {
            vec![edge.start_uv, edge.end_uv]
        };
        built_edges.push(TrimEdgeData {
            start_uv: edge.start_uv,
            end_uv: edge.end_uv,
            curve_3d: curve,
            uv_samples,
        });
    ...
    if built_edges.is_empty() {
        return Err(RgmStatus::InvalidInput);
    }
    let mut loop_data = TrimLoopData {
        edges: built_edges,
        is_outer: loop_input.is_outer,
    };
    ensure_loop_closed(&mut loop_data, tol);
    if trim_loop_polyline(&loop_data).len() < 4 {
        return Err(RgmStatus::InvalidInput);
    }
    Ok(loop_data)
}

fn split_trim_edge_samples(
    edge: &TrimEdgeData,
    split_t: f64,
    tol: f64,
) -> (RgmUv2, Vec<RgmUv2>, Vec<RgmUv2>) {
    ...
    let mut total = 0.0;
    let mut lengths = Vec::with_capacity(samples.len().saturating_sub(1));
    for seg in samples.windows(2) {
        let len = uv_distance(seg[0], seg[1]);
        lengths.push(len);
        total += len;
    }
    if total <= tol {
        ...
        return (
            split_uv,
            vec![edge.start_uv, split_uv],
            vec![split_uv, edge.end_uv],
        );
    }

    let mut target = split_t.clamp(0.0, 1.0) * total;
    let mut split_uv = edge.end_uv;
    let mut split_seg = lengths.len().saturating_sub(1);
    let mut local_alpha = 1.0;
    for (idx, seg_len) in lengths.iter().copied().enumerate() {
        if target <= seg_len || idx + 1 == lengths.len() {
            split_seg = idx;
            if seg_len <= tol {
                local_alpha = 0.0;
            } else {
                local_alpha = (target / seg_len).clamp(0.0, 1.0);
            }
            split_uv = uv_lerp(samples[idx], samples[idx + 1], local_alpha);
            break;
        }
        target -= seg_len;
    }
    ...
    left.extend_from_slice(&samples[..=split_seg]);
    if uv_distance(*left.last().unwrap_or(&split_uv), split_uv) > tol {
        left.push(split_uv);
    } else if let Some(last) = left.last_mut() {
        *last = split_uv;
    }

    let mut right = vec![split_uv];
    if local_alpha < 1.0 - 1e-12 {
        right.push(samples[split_seg + 1]);
    }
    right.extend_from_slice(&samples[(split_seg + 2)..]);

    (
        split_uv,
        normalize_trim_edge_samples(edge.start_uv, split_uv, left, tol),
        normalize_trim_edge_samples(split_uv, edge.end_uv, right, tol),
    )
}
