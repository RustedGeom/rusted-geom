...
impl SatWriter {
    ...
    // ── High-level: emit a full B-rep body ───────────────────────────────
    ...
    fn add_brep(
        ...
        let nt = brep.trims.len();
        let nl = brep.loops.len();
        let nf = brep.faces.len();
        let ns = brep.shells.len();
        let n_sol = brep.solids.len();

        let off_vertex = base;
        let off_edge = off_vertex + nv;
        let off_point = off_edge + ne;
        let off_coedge = off_point + nv;
        let off_curve = off_coedge + nt;
        let off_loop = off_curve + ne;
        let off_face = off_loop + nl;
        let off_surface = off_face + nf;
        let off_shell = off_surface + nf;
        let off_lump = off_shell + ns;
        let off_body = off_lump + n_sol.max(1);

        let sat_r = |off: usize, i: usize| -> String {
            format!("${}", off + i)
        };

        // ── Vertices ─────────────────────────────────────────────────────
        for (i, v) in brep.vertices.iter().enumerate() {
            let edge_ref = if let Some(&first_edge) = v.incident_edges.first() {
                sat_r(off_edge, first_edge.index())
            } else {
                Self::null_ref().to_string()
            };
            let point_ref = sat_r(off_point, i);
            let rec = format!("vertex {} $-1 {} #", edge_ref, point_ref);
            self.push(rec);
        }

        // ── Edges ────────────────────────────────────────────────────────
        for (i, edge) in brep.edges.iter().enumerate() {
            let coedge_ref = if let Some(&first_trim) = edge.trims.first() {
                sat_r(off_coedge, first_trim.index())
            } else {
                Self::null_ref().to_string()
            };
            let vs = sat_r(off_vertex, edge.v_start.index());
            let ve = sat_r(off_vertex, edge.v_end.index());
            let curve_ref = sat_r(off_curve, i);
            let rec = format!(
                "edge {} {} {} {} forward #",
                coedge_ref, vs, ve, curve_ref
            );
            self.push(rec);
        }

        // ── Points (one per vertex) ──────────────────────────────────────
        for v in brep.vertices.iter() {
            let pt = v.point_3d.unwrap_or(RgmPoint3 {
                x: v.uv.u as f64,
                y: v.uv.v as f64,
                z: 0.0,
            });
        ...
        // ── Coedges (one per trim) ──────────────────────────────────────
        for (trim_raw_idx, trim) in brep.trims.iter().enumerate() {
            let loop_data = &brep.loops[trim.loop_id];
            let trim_pos = loop_data
                .trims
                .iter()
                .position(|t| t.index() == trim_raw_idx)
                .unwrap_or(0);
            let n_trims = loop_data.trims.len();
            let next_pos = (trim_pos + 1) % n_trims;
            let prev_pos = (trim_pos + n_trims - 1) % n_trims;

            let next_trim_idx = loop_data.trims[next_pos].index();
            let prev_trim_idx = loop_data.trims[prev_pos].index();

            let next_ref = sat_r(off_coedge, next_trim_idx);
            let prev_ref = sat_r(off_coedge, prev_trim_idx);
            let loop_ref = sat_r(off_loop, trim.loop_id.index());
            let edge_ref = sat_r(off_edge, trim.edge.index());
            let sense = if trim.reversed { "reversed" } else { "forward" };
        ...
        // ── Edge curves (one per edge, straight-line fallback) ──────────
        for edge in brep.edges.iter() {
            if let Some(curve_handle) = edge.curve_3d {
                if let Some(GeometryObject::Curve(curve_data)) =
                    state.objects.get(&curve_handle.0)
                {
                    if let Some(nurbs) = curve_canonical_nurbs(curve_data) {
                        let spline_idx = self.write_spline_curve(&nurbs.core);
                        let rec = format!(
                            "exactcur-curve {} $-1 #",
                            Self::sat_ref(spline_idx)
                        );
                        self.entities.last_mut().unwrap().record = rec;
                        continue;
                    }
                }
            }
            let v_start = &brep.vertices[edge.v_start];
            let v_end = &brep.vertices[edge.v_end];
            let p0 = v_start.point_3d.unwrap_or(RgmPoint3 {
                x: v_start.uv.u as f64,
                y: v_start.uv.v as f64,
                z: 0.0,
            });
            let p1 = v_end.point_3d.unwrap_or(RgmPoint3 {
                x: v_end.uv.u as f64,
                y: v_end.uv.v as f64,
                z: 0.0,
            });
            let mut s = String::with_capacity(128);
            s.push_str("straight-curve $-1 ");
            sat_push_float(&mut s, p0.x);
            s.push(' ');
            sat_push_float(&mut s, p0.y);
            s.push(' ');
            sat_push_float(&mut s, p0.z);
            s.push(' ');
            let dx = p1.x - p0.x;
            let dy = p1.y - p0.y;
            let dz = p1.z - p0.z;
            let len = (dx * dx + dy * dy + dz * dz).sqrt().max(1e-30);
            sat_push_float(&mut s, dx / len);
            s.push(' ');
            sat_push_float(&mut s, dy / len);
            s.push(' ');
            sat_push_float(&mut s, dz / len);
            s.push_str(" #");
            self.push(s);
        }

        // ── Loops ────────────────────────────────────────────────────────
        for (i, loop_data) in brep.loops.iter().enumerate() {
            let face_ref = sat_r(off_face, loop_data.trims.first().and_then(|tid| brep.trims.get(*tid)).map(|t| t.face.index()).unwrap_or(0));
            let first_coedge = if let Some(&first) = loop_data.trims.first() {
                sat_r(off_coedge, first.index())
            } else {
                Self::null_ref().to_string()
            };
            let _ = i;
            let rec = format!("loop $-1 {} {} #", face_ref, first_coedge);
            self.push(rec);
        }

        // ── Faces ────────────────────────────────────────────────────────
        for (i, face) in brep.faces.iter().enumerate() {
            let first_loop = if let Some(&first) = face.loops.first() {
                sat_r(off_loop, first.index())
            } else {
                Self::null_ref().to_string()
            };
            ...
            let shell_idx = brep
                .shells
                .iter()
                .position(|sh| sh.faces.iter().any(|f| f.index() == i))
                .unwrap_or(0);
            let shell_ref = sat_r(off_shell, shell_idx);
            let surface_ref = sat_r(off_surface, i);
            let sense = if face.orientation >= 0 {
                "forward"
            } else {
                "reversed"
            };
            ...
            let rec = format!(
                "face {} {} {} {} double #",
                shell_ref, first_loop, surface_ref, sense
            );
            self.push(rec);
        }
        ...
        for face in brep.faces.iter() {
            if let Some(GeometryObject::Surface(surf)) =
                state.objects.get(&face.surface.0)
            {
                let spline_idx = self.write_spline_surface(&surf.core, &surf.transform);
                let rec = format!("spline-surface {} #", Self::sat_ref(spline_idx));
                self.entities.last_mut().unwrap().record = rec;
            } else {
                self.push("cone-surface $-1 0 0 0 0 0 1 1 0 0 1 #".to_string());
            }
        }

        // ── Shells ───────────────────────────────────────────────────────
        let lump_ref_for_shell = |_i: usize| -> String {
            if n_sol > 0 {
                sat_r(off_lump, 0)
            } else {
                sat_r(off_lump, 0)
        ...
        for (i, shell) in brep.shells.iter().enumerate() {
            let first_face = if let Some(&first) = shell.faces.first() {
                sat_r(off_face, first.index())
            } else {
                Self::null_ref().to_string()
            };
            let lump_ref = lump_ref_for_shell(i);
            let rec = format!("shell {} $-1 {} #", lump_ref, first_face);
            self.push(rec);
        }

        // If there are no shells, write a synthetic one.
        if ns == 0 && nf > 0 {
            let lump_ref = sat_r(off_lump, 0);
            let first_face = sat_r(off_face, 0);
            let rec = format!("shell {} $-1 {} #", lump_ref, first_face);
            self.push(rec);
        }
        ...
        if n_sol > 0 {
            for (i, solid) in brep.solids.iter().enumerate() {
                let first_shell = if let Some(&first) = solid.shells.first() {
                    sat_r(off_shell, first.index())
                } else {
                    Self::null_ref().to_string()
                };
                let body_ref = sat_r(off_body, i);
                let rec = format!("lump {} $-1 {} #", body_ref, first_shell);
                self.push(rec);
            }
            for (i, _solid) in brep.solids.iter().enumerate() {
                let lump_ref = sat_r(off_lump, i);
                let rec = format!("body $-1 {} $-1 #", lump_ref);
                self.push(rec);
            }
        } else {
            let shell_ref = if ns > 0 {
                sat_r(off_shell, 0)
            } else if nf > 0 {
                sat_r(off_shell, 0)
            } else {
            ...
            let body_idx = off_body;
            let body_ref_s = format!("${body_idx}");
            let rec = format!("lump {} $-1 {} #", body_ref_s, shell_ref);
            self.push(rec);
            let lump_ref = sat_r(off_lump, 0);
            let rec = format!("body $-1 {} $-1 #", lump_ref);
            self.push(rec);
...
pub(crate) fn export_sat_text(
    ...
    for &obj_id in object_ids {
        ...
        match obj {
            ...
            GeometryObject::Face(data) => {
                if let Some(GeometryObject::Surface(surf)) =
                    state.objects.get(&data.surface.0)
                {
                    writer.add_standalone_surface(&surf.core, &surf.transform);
                }
            }
            GeometryObject::Brep(brep) => {
                writer.add_brep(brep, &state);
            }
